#!/usr/bin/env python3
"""Validate frozen client release manifest against release-track contract."""
...
EXPECTED_SCHEMA = "kaigi-client-release-manifest/v1"
...
def main() -> int:
    ...
    release_train = require_non_empty_str(payload, "release_train", "client release manifest")
    if release_train != EXPECTED_RELEASE_TRAIN:
        raise RuntimeError(
            f"client release manifest: release_train must be {EXPECTED_RELEASE_TRAIN}"
        )
    ...
    for artifact in artifacts:
        ...
        if workspace_id in seen_workspace_ids:
            raise RuntimeError(f"client release manifest: duplicate workspace_id {workspace_id}")
        if workspace_id not in track_by_workspace:
            raise RuntimeError(
                f"client release manifest: unknown workspace_id {workspace_id}"
            )
        ...
        artifact_uri = require_non_empty_str(artifact, "artifact_uri", f"artifact {workspace_id}")
        if workspace_id == "web":
            web_workspace_count += 1
            if distribution_channel != "ipfs":
                raise RuntimeError("artifact web: distribution_channel must be ipfs")
            if not artifact_uri.startswith("ipfs://"):
                raise RuntimeError("artifact web: artifact_uri must start with ipfs://")
        else:
            native_workspace_count += 1
            if artifact_uri.startswith("ipfs://"):
                raise RuntimeError(
                    f"artifact {workspace_id}: native artifact_uri must not be ipfs://"
                )

        artifact_sha256 = require_non_empty_str(
            artifact, "artifact_sha256", f"artifact {workspace_id}"
        )
        if not SHA256_RE.fullmatch(artifact_sha256):
            raise RuntimeError(
                f"artifact {workspace_id}: artifact_sha256 must match {SHA256_RE.pattern}"
            )

        for field in ("signature_ref", "sbom_ref", "provenance_ref"):
    ...
    if missing_workspace_ids:
        raise RuntimeError(
            "client release manifest missing workspace entries: "
            + ", ".join(missing_workspace_ids)
        )

    if ordered_workspace_ids != sorted(ordered_workspace_ids):
        raise RuntimeError("client release manifest artifacts must be sorted by workspace_id")

    if web_workspace_count != 1:
        raise RuntimeError("client release manifest must include exactly one web artifact entry")
    if native_workspace_count == 0:
        raise RuntimeError("client release manifest must include native artifact entries")

    print(
        "[OK] client release manifest valid "
